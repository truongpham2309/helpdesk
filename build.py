...
def build_flutter_windows(version, features, skip_portable_pack, quicksupport=False):
    ...
    system2(
        f'{python_exe} ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/helpdesk.exe')

    os.chdir('../..')
    if os.path.exists('./helpdesk_portable.exe'):
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/helpdesk.exe target/release/Helpdesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\helpdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/Helpdesk.exe {res_dir}')
        os.chdir('libs/portable')
